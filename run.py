"""
Minimal entry point for PyInstaller - excludes heavy dependencies at import time
"""
    ...
    try:
                ...
                for (ox1, oy1, ox2, oy2, label) in objects:
                    for person_box in persons:
                        if is_inside([ox1, oy1, ox2, oy2], person_box):
                            ...
                            cv2.putText(frame, label, (ox1, oy1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2)
                            break
        ...
        def display_frames():
            while True:
                ...
                if frame is None:
                    break
                cv2.imshow('Real-Time AI Face & Object Detection', frame)
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    frame_queue.put(None)
                    break

        processor = Thread(target=process_frames, daemon=True)
        display = Thread(target=display_frames, daemon=True)
